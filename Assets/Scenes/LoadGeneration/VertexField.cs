...
public class VertexField
{
    ...
    public bool IsOtherVertexExist(Vector3 position, Vector3 prevPosition, Streamline exceptStreamline, float distance)
    {
        var rangedVertices = GetRangedBoundingBox(position, distance);
        ...
        foreach (var vertex in rangedVertices)
        {
            ...
            if (vertex.NextVertex == null) continue;

            if (MathUtil.IsCrossLine(vertex.Position, vertex.NextVertex.Position, prevPosition, position))
            {
                return true;
            }
        }
    ...
    public Streamline.Vertex FindNearestVertex(Vector3 position, float maxDistance)
    {
        var rangedBoundingBox = GetRangedBoundingBox(position, maxDistance);
        float minDistance = float.MaxValue;
        Streamline.Vertex minDistanceVertex = null;
        
        foreach (var vertex in rangedBoundingBox)
        {
            float distance = Vector3.Distance(position, vertex.Position);
