...
public class StraightPolygon
{
    ...
    public class Edge
    {
        ...
        public List<Vertex> PrevSubVertices = new List<Vertex>();
        public List<Vertex> NextSubVertices = new List<Vertex>();
        ...
        public override bool Equals(object obj)
        {
            ...
            return (PreviousVertex.Position.RefEquals(e.PreviousVertex.Position) && NextVertex.Position.RefEquals(e.NextVertex.Position)) || (PreviousVertex.Position.RefEquals(e.NextVertex.Position) && NextVertex.Position.RefEquals(e.PreviousVertex.Position));
        }
    ...
    private List<Edge> _defaultEdges = new List<Edge>();
    private List<Vertex> _vertices = new List<Vertex>();
    private List<Vertex> _priorityQueue = new List<Vertex>();
    
    public List<Edge> Process(List<Vector3> vertices)
    {
        _InitializeVertices(vertices);
        ...
        while (_priorityQueue.Count != 0)
        {
            ...
            {
                _ProcessEdgeEvent(currentIntersection, results);
            }
    ...
    void _InitializeVertices(List<Vector3> vertices)
    {
        _defaultEdges.Clear();
        _priorityQueue.Clear();
        _vertices.Clear();
        
        List<Vertex> nonConvexIntersection = new List<Vertex>();
        ...
        for (int i = 0; i < vertices.Count; i++)
        {
            ...
            current.DefaultPreviousVertex = prev;
        ...
        {
            Vertex intersection = _CreateNewIntersection(vertex, vertex.PreviousVertex, vertex.NextVertex);
        ...
        foreach (var vertex in nonConvexIntersection)
        {
            ...
            if (findOppositeEdgeResult.DistanceToEdge < vertex.DistanceToEdge)
            {
    ...
    void _ProcessEdgeEvent(Vertex currentIntersection, List<Edge> results)
    {
    ...
    private void _ProcessSplitEvent(Vertex currentIntersection, List<Edge> results)
    {
        ...
        List<Vertex> subPolygon = new List<Vertex> { currentVertex };
        Vertex subPolygonIterator = currentVertex.NextVertex;
    ...
    Vertex _CreateNewIntersection(Vertex current, Vertex prev, Vertex next)
    {
        ...
        bool isPrevIntersectInside = _IsAngleInsidePosition(previousBisectorIntersectionPoint, prev.PreviousVertex.Position,
        ...
        bool isNextIntersectInside = _IsAngleInsidePosition(nextBisectorIntersectionPoint, current.Position,
    ...
    FindOppositeEdgeResult _FindOppositeEdge(Vertex vertex)
    {
        ...
        {
            Vector3 intersectPoint = _GetIntersectPointWithOppositeEdge(vertex, edge);
            
            if (!_IsVertexInFrontOfEdge(intersectPoint,edge))
            {
                continue;
            }
    ...
    Vector3 _GetIntersectPointWithOppositeEdge(Vertex origin, Edge oppositeEdge)
    {
        Vector3 testIntersectPoint = MathUtil.GetCrossPoint(oppositeEdge.DefaultPreviousVertex.Position, oppositeEdge.DefaultNextVertex.Position, origin.DefaultNextVertex.Position,
            origin.Position);
        
        Vector3 testIntersectBisector = MathUtil.GetBisectorVector3(testIntersectPoint, origin.Position, oppositeEdge.DefaultPreviousVertex.Position);

        return MathUtil.GetCrossPoint(origin.Position, origin.Position + origin.Direction * 2.0f, testIntersectPoint,
            testIntersectPoint + testIntersectBisector * 2.0f);
    }
    
    bool _IsVertexInFrontOfEdge(Vector3 origin, Edge edge)
    {
        var isPrevAngleInside = _IsAngleInside(origin, (edge.DefaultNextVertex.Position - edge.DefaultPreviousVertex.Position).normalized, edge.DefaultPreviousVertex);
        var isNextAngleInside = _IsAngleInside(origin, (edge.DefaultPreviousVertex.Position - edge.DefaultNextVertex.Position).normalized, edge.DefaultNextVertex); 
        return isPrevAngleInside && isNextAngleInside;
    }

    bool _IsAngleInside(Vector3 origin, Vector3 edgeDirection, Vertex edgeVertex)
    {
    ...
    bool _IsAngleInside(Vector3 origin, Vector3 from, Vector3 to, Vector3 center)
    {
    ...
    bool _IsAngleInsidePosition(Vector3 origin, Vector3 prevPosition, Vector3 centerPosition, Vector3 nextPosition)
    {
        return _IsAngleInside(origin, (prevPosition - centerPosition).normalized,
