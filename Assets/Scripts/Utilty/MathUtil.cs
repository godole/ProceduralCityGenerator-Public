...
public class PolygonSplit
{
    ...
    private static void GetShrinkPolygonInternal(List<List<Vector3>> results, List<Vector3> polygonPoints, float distance)
    {
        ...
        for (int i = 0; i < shrinkPoints.Count; i++)
        {
            ...
            for (int j = 0; j < shrinkPoints.Count - 2; j++)
            {
                ...
                var edgeEventPoint = StraightPolygon.GetIntersectPointWithOppositeEdge(linkedShrinkPoints[i].Position, linkedShrinkPoints[i].NextPoint.Position, bisectorDirections[i], currentCheckEdgePoint.Position, currentCheckEdgePoint.NextPoint.Position);
                float edgeEventDistance = MathUtil.GetLineToPositionDistance(currentCheckEdgePoint.Position, currentCheckEdgePoint.NextPoint.Position, edgeEventPoint);
        ...
        if (eventType == 1)
        {
            ...
            for (int i = 0; i < shrinkPoints.Count; i++)
            {
                ...
                {
                    vertexEventResult.Add(polygonPoints[i] + bisectorDirections[i] * (minShrinkDistance / distanceParents[i]));
                }
            }
        ...
        if (eventType == 2)
        {
            ...
            foreach (var shrinkPoint in leftShrinkPoints)
            {
                ...
                else
                {
                    ...
                    leftShrinkPointResults.Add(polygonPoints[originalIndex] + bisectorDirections[originalIndex] * (minShrinkDistance / distanceParents[originalIndex]));
                }
            }
            ...
            foreach (var shrinkPoint in rightShrinkPoints)
            {
                ...
                {
                    int originalIndex = shrinkPoint.OriginalIndex;
                    rightShrinkPointResults.Add(polygonPoints[originalIndex] + bisectorDirections[originalIndex] * (minShrinkDistance / distanceParents[originalIndex]));
                }
            }
            ...
            // results.Add(leftShrinkPointResults);
            // results.Add(rightShrinkPointResults);
            return;
        }
